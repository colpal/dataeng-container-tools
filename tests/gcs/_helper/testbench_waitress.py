# Source: https://github.com/googleapis/storage-testbench/blob/v0.54.0/testbench_waitress.py
...
def testbench_create_server(application, map=None, _start=True, _sock=None, _dispatcher=None, **kw):
    # This check is only intended to support testing of values populated in the map.
    if map is None:
        map = {}
    server = create_server(application=application, map=map, _start=_start, _sock=_sock, _dispatcher=_dispatcher, **kw)

    for key in map:
        if map[key].__class__.__name__ == "TcpWSGIServer" or map[key].__class__.__name__ == "UnixWSGIServer":
