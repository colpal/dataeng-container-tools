...
def test_input_output_multiple_files(mock_file_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(mock_file_path),
            "--input_bucket_names", "bucket1", "bucket2",
            "--input_paths", "path1", "path2",
            "--input_filenames", "file1.csv", "file2.csv",
            "--output_bucket_names", "out-bucket1", "out-bucket2",
            "--output_paths", "out-path1", "out-path2",
            "--output_filenames", "out-file1.csv", "out-file2.csv",
        ],
        capture_output=True,
        text=True,
        check=False,
...
def test_mismatched_input_lengths(mock_file_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            ...
            str(mock_file_path),
            "--input_bucket_names", "bucket1", "bucket2",
            "--input_paths", "path1",  # Only one path for two buckets
            "--input_filenames", "file1.csv", "file2.csv",
            "--output_bucket_names", "output-bucket",
            "--output_paths", "output/path",
            "--output_filenames", "result.csv",
